    ...
    <div className="absolute inset-x-0 top-0 z-10 md:left-auto m-[2px]">
      <div className="rounded-t-[10px] md:rounded-lg flex items-stretch justify-end px-2 py-1 md:m-1">
        <button
          ...
          className={`bg-black bg-opacity-20 text-white rounded-md flex mr-1 px-3 py-2 text-xs font-medium`}>
          Preview
          </button>
        <button
          onClick={() => setShowCode(true)}
          className={`${showCode ? "bg-white bg-opacity-5" : ""} bg-opacity-20 text-white rounded-md flex mr-1 px-3 py-2 text-xs font-medium`}>
          Code
          </button>
      </div>
    </div>
